"""Risk management — position sizing based on QuantAgent's risk-reward output."""
import logging
...
def calculate_position_size(
    ...
    Risk RISK_PER_TRADE (2%) of equity per trade.
    Position size is determined by distance to stop-loss.
...
def check_drawdown(equity: float, peak: float) -> bool:
    ...
    Stops trading when drawdown from peak exceeds MAX_DRAWDOWN (10%).
    """
    if peak == 0:
        return True
    dd = (peak - equity) / peak
