"""
Bridge between QuantAgent (signal) and Hyperliquid (execution).

QuantAgent outputs (via final_state):
  - final_trade_decision: string containing LONG or SHORT + rationale
  - indicator_report: string from IndicatorAgent
  - pattern_report:   string from PatternAgent
  - trend_report:     string from TrendAgent

This bridge:
  1. Fetches latest OHLCV from Hyperliquid
  2. Calculates ATR for stop/TP sizing
  3. Runs QuantAgent pipeline
  4. Parses the decision (direction + risk-reward)
  5. Calculates ATR-based stop-loss and take-profit
  6. Returns a fully formed signal dict ready for execution
"""
import sys
import re
import os
...
class QuantAgentBridge:
    ...
    def get_signal(self) -> dict | None:
        """
        Run QuantAgent analysis on latest BTC data from Hyperliquid.
        ...
        Returns:
            ...
            None if signal generation fails or no clear direction.
        """
        try:
            # 1. Fetch latest candles (list of row dicts with lowercase keys)
            candles = self.data_feed.get_candles(TRADING_PAIR, TIMEFRAME, QUANTAGENT_LOOKBACK)
            candles_df = pd.DataFrame(candles)

            # 2. Calculate ATR for stop/TP sizing
            ...
            # 3. Convert to QuantAgent's column-oriented dict format (capitalized keys + Datetime)
            kline_data = {
                "Datetime": pd.to_datetime(candles_df["timestamp"], unit="ms").dt.strftime("%Y-%m-%d %H:%M:%S").tolist(),
            ...
            # 6. Build rationale string (used for DegenClaw forum posts)
    ...
    def _parse_direction(self, decision_text: str) -> str | None:
        ...
        # Look for JSON "decision" field first (most reliable)
    ...
    def _build_rationale(self, direction: str, rr: float, state: dict) -> str:
        ...
        decision   = state.get("final_trade_decision", "No decision text")
